...
/*   Updated: 2023/09/14 01:04:42 by donson           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_buf(char *argv, int first, int buf_size)
{
	...
	fd = open(argv, O_RDONLY);
	if (fd == -1 && free_func_two(buf_first, buf))
		return (0);
	read(fd, buf_first, first);
	if (read(fd, buf, buf_size) < buf_size && free_func_two(buf_first, buf))
		return (0);
	if (!(read(fd, chk, 2) < 2) && free_func_two(buf_first, buf))
		return (0);
...
char	**parse(char *argv, int size[2], char character[3])
{
	...
	size[0] = line;
	free(arr);
	arr = get_buf(argv, first, ((size[1] + 1) * size[0]));
	if (arr == 0 && free_func(arr))
		return (0);
	free(arr);
	return (ft_split(arr, "\n"));
}
